...
bool BandMaxSAT::parse_parameters2(int argc, char **argv)
{
    int i = 0;
    int temp_para = 0;
    // cout << "c test:" << endl;
    for (i = 1; i < argc; i++)
    {
        ...
        else if (0 == strcmp(argv[i], "-lambda_"))
        {
            i++;
            if (i >= argc)
                return false;
        ...
        else if (0 == strcmp(argv[i], "-armnum"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%d", &ArmNum);
        }
        else if (0 == strcmp(argv[i], "-backward_step"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%d", &backward_step);
        }
...
void BandMaxSAT::ioh_recording_init()
{
...
void BandMaxSAT::ioh_recording_open_file(char *filename)
{
