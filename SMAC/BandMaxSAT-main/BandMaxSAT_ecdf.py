...
def ecdf(config: Configuration, ins: str, seed: int = 0, rand_tag: int = 0):
    ...
    system(f"./BandMaxSAT-ecdf {ins} 1 -cpu_time {cpu_time} {format_instance(config)} > ./{pre_Tag}_tmp_{random_number}.out" )

    out_file = open(join(current_path,f'./{pre_Tag}_tmp_{random_number}.out'))
...
def found_f(config: Configuration, ins: str, seed: int = 0, rand_tag: int = 0):
    ...
    random.shuffle(rand_ins)
    random_number = random.randint(1, 10000000)
    system(f"./BandMaxSAT-best-f {ins} 1 -cpu_time {cpu_time} {format_instance(config)} > ./{pre_Tag}_tmp_{random_number}.out" )
