...
bool MaxFPS::parse_parameters2(int argc, char **argv)
{
    int i = 0;
    int temp_para = 0;
    // cout << "c test:" << endl;
    for (i = 1; i < argc; i++)
    {
        ...
        else if (0 == strcmp(argv[i], "-sc_num"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%d", &sc_num);
        }
        else if (0 == strcmp(argv[i], "-sv_num"))
        {
            i++;
            if (i >= argc)
                return false;
            sscanf(argv[i], "%d", &sv_num);
        }
    }
    return true;
}
...
void MaxFPS::ioh_recording_init()
{
...
void MaxFPS::ioh_recording_open_file(char *filename)
{
