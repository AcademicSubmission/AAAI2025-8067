...
def format_instance(config: Configuration):
    ...
    return f'-bms_num {config["bms_num"]} -hard_sp {config["hard_sp"]} \
            -soft_weight_threshold {config["soft_weight_threshold"]} \
                  -h_inc {config["h_inc"]} -sv_num {config["sv_num"]} -sc_num {config["sc_num"]}'
...
def ecdf(config: Configuration, ins: str, seed: int = 0, rand_tag: int = 0):
    ...
    system(f"./MaxFPS-ecdf {ins} 1 -cpu_time {cpu_time} {format_instance(config)} > ./{pre_Tag}_tmp_{random_number}.out" )
...
def found_f(config: Configuration, ins: str, seed: int = 0, rand_tag: int = 0):
    ...
    r = pd.read_csv('best_mse23-uw.csv')
    ...
    random.shuffle(rand_ins)
    random_number = random.randint(1, 10000000)
    system(f"./MaxFPS-best-f {ins} 1 -cpu_time {cpu_time} {format_instance(config)} > ./{pre_Tag}_tmp_{random_number}.out" )
