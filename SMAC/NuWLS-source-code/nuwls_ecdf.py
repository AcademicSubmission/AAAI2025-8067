...
def format_instance(config: Configuration):
    if "h_inc" in config.keys():
        return f'-bms_num {config["bms_num"]} -hard_sp {config["hard_sp"]} -soft_sp {config["soft_sp"]} \
            -soft_weight_threshold {config["soft_weight_threshold"]} -h_inc {config["h_inc"]} -s_inc {config["s_inc"]} -coe {config["coe"]}'
    else:
        return f'-bms_num {config["bms_num"]} -hard_sp {config["hard_sp"]} -soft_sp {config["soft_sp"]} \
            -soft_weight_threshold {config["soft_weight_threshold"]}' 
...
def ecdf(config: Configuration, ins: str, seed: int = 0, rand_tag: int = 0):
    ...
    rand_ins = list(ins)
    random.seed(time.time() + float(config["soft_sp"]) + rand_tag)
    random.shuffle(rand_ins)
    random_number = random.randint(1, 10000000)
    
    system(f"./nuwls-ecdf {ins} 1 -cpu_time {cpu_time} {format_instance(config)} > ./{pre_Tag}_tmp_{random_number}.out" )
...
def found_f(config: Configuration, ins: str, seed: int = 0, rand_tag: int = 0):
    ...
    random.shuffle(rand_ins)
    random_number = random.randint(1, 10000000)
    system(f"./nuwls-best-f {ins} 1 -cpu_time {cpu_time} {format_instance(config)} > ./{pre_Tag}_tmp_{random_number}.out" )
